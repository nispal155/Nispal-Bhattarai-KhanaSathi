...
export default function StaffPage() {
    ...
    const fetchStaff = async () => {
        try {
            setLoading(true);
            const response = await get<{ success: boolean; data: StaffMember[] }>('/restaurant/my-restaurant/staff');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setSubmitting(true);
            if (editingId) {
                await put(`/restaurant/my-restaurant/staff/${editingId}`, formData);
                toast.success("Staff updated successfully!");
            } else {
                await post('/restaurant/my-restaurant/staff', formData);
    ...
    const handleDelete = async (id: string) => {
        ...
        try {
            await del(`/restaurant/my-restaurant/staff/${id}`);
    ...
    return (
        <div className="p-8">
            <div className="max-w-6xl mx-auto">
                {/* Header Section (Inline with main content) */}
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-800">Staff Management</h1>
                        <p className="text-gray-500">Manage your restaurant staff team</p>
                    </div>
                    <button
                ...
                {loading ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {staff.map((member) => (
                            <div key={member._id} className="bg-white rounded-2xl shadow-sm p-6 border border-gray-100 hover:shadow-md transition">
                                <div className="flex items-start justify-between mb-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold uppercase ${getRoleColor(member.role)}`}>
                                                {member.role}
                                            </span>
                                        </div>
                                    </div>
                                ...
                                <div className="flex items-center gap-2 pt-4 border-t border-gray-100">
                                    <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${member.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600'}`}>
                                        {member.status}
                                    </span>
